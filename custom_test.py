...
df_save_path ='results/mid/tested_data(song-light-0-norm).csv'
dropout_rate = 0.1
...
with torch.no_grad():
    for text, label in tqdm(zip(texts, labels)):
        ...
        prediction_mean, prediction_std, predicted_label, bert_hidden_state = model.monte_carlo_forward(input_ids, attention_mask, num_samples, embedding_idx=0, method="norm2")
