...
for rate in range(20, 60, 10):
    ...
    model = BertWithMCDO(dropout_rate=rate)
    ...
    print(f"Training the model... rate : {rate}")
    model.train()
    ...
    plt.savefig(f'outputs/rates/loss_graph({int(rate * 100)}).png')
    ...
    plt.savefig(f'outputs/accuracy_graph({int(rate * 100)}).png')
    ...
    plt.savefig(f'outputs/training_acc_loss({int(rate * 100)}).png')
    ...
    # loop with tqdm
        for text, label in tqdm(zip(texts, labels)):
            ...
            with torch.no_grad():
                ...
                if int(predicted_labels) == label:
                    correct_cnt += 1'''
    
    print(f'real test accuracy at rate {rate}: ', correct_cnt / len(texts))


    model.save(f"models/rates/bert_monte_carlo_dropout_model(multi-case)({rate}).pth")